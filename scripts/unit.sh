...
go test -tags=unit -json -cover -coverprofile="./cover.out" ./... | tparse || true
go tool cover -html="./cover.out" -o "./test/results/html-reports/backend-coverage.html"
go test -tags=unit -v -covermode=atomic ./... 2>&1 | go-junit-report >./test/results/junit/backend.xml
